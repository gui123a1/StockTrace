...
| 已完成 | 全站统一区间体系 | periods.py 统一解析器；板块资金原生 5d/10d 多周期；走势/国家队支持自定义起止；新增大盘资金流与北向区间端点；ETF 详情增 1w；机构持股支持指定季度 |

## 完成记录

### 2026-08-30：全站统一区间体系（与并行工作线协同完成）

- **背景**：用户要求"所有数据都可看区间（单日/3日/5日/周/月/季/半年/年 + 自定义），且各数据用自己合适的档位"。实施中出现两条工作线并行改同一批文件，经确认采用"主线推进 + 本线验收补全"模式收口。
- **数据-区间适配结论**：日频历史型（国家队资金流/大盘资金流/北向/指数走势）支持全档位+自定义，受上游深度限制处如实标注 coverage_start；当日横截面型（板块/雷达现况）仅当日，板块多周期走东财排行接口原生的 day/5d/10d（**无 3d**，400 为正确行为）；季频型（机构持股）支持 `?quarter=` 指定报告期；实时快照（指数现价/涨跌家数）无区间概念。
- **统一解析器**：`market/periods.py`（预设档位+自定义起止+跨度上限），新接口一律经它声明档位；大盘资金流窗口（`/api/market/market-flow/`，自有 push2his 抓取）与北向窗口（`/api/market/northbound/`）接入。
- **并行线交付**：板块资金原生多周期（`_SECTOR_RANK_INDICATORS`，period=day/5d/10d）、走势 period/旧 days/自定义三选一（自定义最长约 3 年）、国家队 1d/3d/5d 交易日档与 6m 及自定义校验、ETF 详情增 1w、前端 5 个页面的周期选择 UI。
- **验收补全（本线）**：大盘资金流/北向窗口端点接线与聚合测试、机构持股 `?quarter=` 参数与 400 校验、periods 解析器单测（预设窗口/自定义/越界/禁用档位）。
- 验证：`manage.py test stocks` 39 通过、ruff/eslint/build 通过；浏览器实测走势页周期切换与自定义区间、板块 `period=5d`（结构正确，数据待当晚中断的东财线路恢复）、国家队页区块正常。大盘资金流/北向端点已就绪，前端入口待下一轮接入总览/机构页。
