...
| 已完成 | 国家队 ETF 区间资金流向 | 新增 `/api/market/national-etf/flow/`，支持近1周/1月/3月/今年以来区间的主力净流入聚合与图表；雷达/国家队页头部标注数据截至日期 |

## 完成记录

### 2026-08-30：国家队 ETF 区间资金流向

- **需求**：用户要的是"某一段时间资金流动的变化"（例：国家队 3 月以来的变化），而非逐日快照积累。
- **数据源**：东财 push2his 历史资金流日线（`market/etf_flow.py`，原生 requests + https 退避重试 + http 兜底，绕开 akshare——本机代理对该线路 TLS 间歇性干扰）。实测深度恰为最近 120 个交易日；ETF 行字段与股票不同（日期,主力,小单,中单,大单,超大单,占比…,收盘,涨跌幅），已按"主力=超大单+大单"校验解析。
- **后端**：`get_national_team_flow(period)` 按自然日窗口聚合 18 只观察 ETF 的主力净流入（区间合计、净流入/流出天数、区间涨跌、合计日序列）；整包仅在 18 只全部拉齐时缓存，部分失败由单只缓存渐进收敛；线路上游不可达时探针快速失败（不缓存）。
- **前端**：国家队页新增"区间资金流向"区块（近1周/1月/3月/今年以来切换、区间汇总条、每股净流入横向柱图）；国家队页与份额雷达页头部新增"数据截至"日期徽标。
- 验证：`manage.py test stocks` 23 通过（含 ETF 行解析"主力=超大+大"校验、窗口聚合、缓存命中、非法 period 400）；ruff/eslint/build 通过；浏览器实测新区块渲染与真实数据（510050 近3月 +8.47 亿，覆盖 2026-03-09~2026-08-28）。当晚本机代理到东财线路大面积中断，其余标的待网络恢复后点时间段自动补齐；VPS 国内线路不受影响。
